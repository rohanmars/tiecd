...
class KubernetesHandler implements DeployHandler {
  ...
  Future<void> login(Environment environment) async {
    ...
    if (environment.apiToken.isNullOrEmpty &&
        environment.apiClientCert.isNullOrEmpty &&
        environment.apiClientKey.isNullOrEmpty) {
      throw TieError(
    ...
    if (!File(_kubeConfigFilename!).existsSync()) {
      if (environment.apiConfig.isNotNullNorEmpty) {
    ...
    if (environment.apiClientCA != null) {
      context.setTrustedCertificatesBytes(
          base64.decode(environment.apiClientCA!));
    }
    ...
    if (environment.apiToken.isNotNullNorEmpty) {
      ...
      context.useCertificateChainBytes(
          base64.decode(environment.apiClientCert!));
      context.usePrivateKeyBytes(
          base64.decode(environment.apiClientKey!));
  ...
  Future<void> logoff() async {
    try {
      ...
      Log.error('error logging off $error');
    }
  }
            ...
            if (auth.isNotNullNorEmpty) {
              ...
              var dockerSecret = {
                ...
                "metadata": {
                  "name": name,
                  "labels": {
                    "app.kubernetes.io/component": "regcred",
                    "app.kubernetes.io/instance": "regcred",
                    "app.kubernetes.io/managed-by": "tiecd",
                    "app.kubernetes.io/name": "regcred",
                    "tiecd/scope": "namespace"
                  }
                },
                "data": {
              ...
              deployContext.environment.deployedArtifacts[namespace]!.add('secret/$name');
            }
          }
        }
      } else {
    ...
    if (deployContext.app.deploy!.helmChart != null) {
      ...
      if (chartLog != null) {
        ...
        try {
          ...
          await buildNamespace(deployContext, namespace: chart.namespace);
  ...
  void _addTiecdResource(Map<String,String> filterLabels, Map<String,String> labels, Set<String> resources, String resourceName ) {
    var match = true;
    for (MapEntry<String, String> item in filterLabels.entries) {
      if (!labels.containsKey(item.key) || labels[item.key] != item.value) {
        match = false;
        break;
      }
    }
  ...
  Future<void> _cleanResources(String namespace, Map<String,String> filter, Set<String> deployedArtifacts) async {
    ...
    for (api_core_v1.Secret secret in secrets.items) {
      ...
      if (labels != null) {
        _addTiecdResource(filter,labels, resources, 'secret/${secret.metadata!.name}');
      }
    }
    // configmaps
    ...
    for (api_core_v1.ConfigMap configMap in configMaps.items) {
      ...
      if (labels != null) {
        _addTiecdResource(filter,labels, resources, 'configmap/${configMap.metadata!.name}');
      }
    }
    // services
    api_core_v1.ServiceList services = await _kubernetesClient!.listCoreV1NamespacedService(namespace: namespace);
    for (api_core_v1.Service service in services.items) {
      var labels = service.metadata!.labels;
      if (labels != null) {
        _addTiecdResource(filter,labels, resources, 'service/${service.metadata!.name}');
      }
    }
    // deployments
    api_apps_v1.DeploymentList deployments = await _kubernetesClient!.listAppsV1NamespacedDeployment(namespace: namespace);
    for (api_apps_v1.Deployment deployment in deployments.items) {
      var labels = deployment.metadata!.labels;
      if (labels != null) {
        _addTiecdResource(filter,labels, resources, 'deployment/${deployment.metadata!.name}');
      }
    }
    // serviceaccount
    api_core_v1.ServiceAccountList serviceAcounts = await _kubernetesClient!
        .listCoreV1NamespacedServiceAccount(namespace: namespace);
    for (api_core_v1.ServiceAccount serviceAccount in serviceAcounts.items) {
      var labels = serviceAccount.metadata!.labels;
      if (labels != null) {
        _addTiecdResource(filter,labels, resources, 'serviceaccount/${serviceAccount.metadata!.name}');
      }
    }

    for (var element in resources) {
      if (!deployedArtifacts.contains(element)) {
        List<String> parts = element.controlledSplit('/');
        switch (parts[0]) {
          case 'secret':
            {
              if (config.autoGeneratedCleanupDryRun) {
                Log.info(
          ...
          case 'service':
            {
              if (config.autoGeneratedCleanupDryRun) {
                Log.info(
                    'dry run - deleting service ${parts[1]} in $namespace namespace');
              } else {
                Log.info(
                    'deleting service ${parts[1]} in $namespace namespace');
                await _kubernetesClient!.deleteCoreV1NamespacedService(
                    name: parts[1], namespace: namespace);
              }
            }
          case 'deployment':
            {
              if (config.autoGeneratedCleanupDryRun) {
                Log.info(
                    'dry run - deleting deployment ${parts[1]} in $namespace namespace');
              } else {
                Log.info(
                    'deleting deployment ${parts[1]} in $namespace namespace');
                await _kubernetesClient!.deleteAppsV1NamespacedDeployment(
                    name: parts[1], namespace: namespace);
              }
            }
          case 'serviceaccount':
            {
              if (config.autoGeneratedCleanupDryRun) {
                Log.info(
                    'dry run - deleting serviceaccount ${parts[1]} in $namespace namespace');
              } else {
                Log.info(
                    'deleting serviceaccoutt ${parts[1]} in $namespace namespace');
                await _kubernetesClient!.deleteCoreV1NamespacedServiceAccount(
                    name: parts[1], namespace: namespace);
              }
            }
        }
  ...
  @override
  Future<void> cleanupResources(DeployContext deployContext) async {
    if (deployContext.config.autoGeneratedCleanup || deployContext.config.autoGeneratedCleanupDryRun) {
      ...
      namespace ??= 'default';
      var filter = {'app.kubernetes.io/name': deployContext.app.name!,
                    'app.kubernetes.io/managed-by': 'tiecd'};
      await _cleanResources(namespace,filter,deployContext.deployedArtifacts);
    }
  }

  //
  @override
  Future<void> cleanupEnvironment(Config config, Environment environment) async {
    if (config.autoGeneratedCleanup || config.autoGeneratedCleanupDryRun) {
      var filter = {'tiecd/scope': 'namespace',
        'app.kubernetes.io/managed-by': 'tiecd'};
      for (MapEntry<String, Set<String>> item in environment.deployedArtifacts
          .entries) {
        await _cleanResources(item.key, filter, item.value);
      }
    }
  }
}
